...
dat_long <-
  ...
  mutate_at(vars(-get_up, -bed_time, -date_ins, -parents, -partner, -brother, -family_others, -friends), as.numeric) %>% 
  clean_names() %>% 
  mutate(month_year = floor_date(date_ins, "month"), week_year = floor_date(date_ins, "week", week_start = 1) ) 
}
